...
- Packet protection keeps its expanded keys instead of rebuilding them for every packet (#230). `protection.KeySet` refers to an AEAD context and a header-protection context through `aead` and `header`, and `protection.bind` points them at a `protection.Contexts` its holder owns and expands the set's keys into it. A set left unbound still expands per packet. `SendKeys` holds its direction's contexts inline, and `ReceiveKeys` holds one AEAD context per generation (previous, current and next) and one header-protection context, since header-protection keys do not change on a key update (RFC 9001 section 6). Installing 1-RTT keys expands them once, a key update expands only the new AEAD key, and the receive side expands its next generation on the first packet that needs it, so forged packets in a new key phase cost at most one expansion per phase. A debug-only `protection.expansion_count()` counts expansions, and a test holds those counts. The Initial, 0-RTT and Handshake levels each take one secret pool chunk for both directions' contexts while their keys are live. Their sets stay unbound until mach-std's typed secret view (mach-std#905) is released, so for now those levels still expand per packet. ChaCha20-Poly1305 has no expanded AEAD context and is unchanged. Measured on one linux-x86_64 machine in a release build, dev (`7da650f`) against this change, with three interleaved exclusive rounds and medians reported. Sealing and opening one 1,200-byte AES-128-GCM 1-RTT packet went from 131.1 to 109.7 microseconds, and a `KeySet` left unbound on this change still took 130.4. ChaCha20-Poly1305 went from 65.7 to 64.9. With hedge dev (`f674583`) holding 5,000 QUIC connections with a one-second keep-alive (hedge#274's probe), a 20 s window took 8,693 ms of CPU before and 6,875 ms after, and 70.6 and 41.6 billion user instructions. That is 43.9 and 34.7 microseconds per UDP datagram delivered on the host. Both hedge builds used crypto 0.21.0 and tls 0.11.0 and differed only in quic.
- **Breaking.** `assembly.init_client` and `init_server` take a `std.memory.buffers.SecretSource` as `secret_chunks`, directly after the chunk source (#230). It must serve the same pool as the chunk source. Init refuses a nil one, or one over another pool, at `STAGE_STORAGE`. Handshake-level key contexts are charged to the connection's account on the send lane. `storage.source` gains `SecretProvider`, `SecretSource`, `take_secret` and `give_secret`, and `supply.classes` names a fourth class, a secret 4,096-byte one (`CLASS_COUNT` is 4). `core.SecretStorage.context_chunks` may be nil, and then every level expands per packet. `core.release_secret_chunks` gives back what a connection still holds.
- `assembly.Connection` is 16,968 bytes, up from 10,640, for the inline 1-RTT contexts: 1,992 bytes to send and 4,008 to receive (#230). `assembly.Storage` stays at 4,560. During the handshake the dialer holds at most six send-lane chunks, up from five, and the listener eight, up from six. The extra chunks are the secret chunks of the handshake levels whose keys are live, and each goes back when its level's keys are discarded. A connection that accepted 0-RTT keeps those keys and their chunk until teardown.
- Dependencies: `[dep.crypto] version = "^0.21"` realized at v0.21.0, for keyed `aes_gcm.Context` and `crypto.cipher.aes` contexts, and `[dep.tls] version = "^0.11"` at v0.11.0, which requires crypto ^0.21 (#230).
