...
- **Breaking.** mach-quic requires mach-std 5.0.0 (#137). Connection storage comes from any `std.memory.buffers.Source`, and `quic.storage.source` is the only module that calls it. `quic.storage.pool` is removed. Account budgets are counted in bytes on two lanes, send and receive, in chunks of `supply.BYTES`.
