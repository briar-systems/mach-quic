...
# reached both long-lived branches once already (the library entry said 0.4.0 while
...
exported=$(awk -F'"' '/^pub val VERSION: str = "/ { print $2; exit }' src/lib/quic.mach)
if [ -z "$manifest" ] || [ -z "$exported" ]; then
  echo "::error::release version is missing: mach.toml=${manifest:-<none>} src/lib/quic.mach=${exported:-<none>}"
  exit 1
fi
if [ "$manifest" != "$exported" ]; then
  echo "::error::release version mismatch: mach.toml=$manifest src/lib/quic.mach=$exported"
  exit 1
fi
